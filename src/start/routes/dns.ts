...
Route.group(() => {
    ...
    Route.resource('/dns/provider.credential', 'DnsProvider/CredentialController')
        ...
        .only(['index', 'store'])
    Route.post('/dns/provider/:providerId/test', 'DnsProvider/ProviderController.test')
    Route.post('/dns/provider/:providerId/credential/:id/test', 'DnsProvider/CredentialController.test')
})
.prefix('/api/v1')
.middleware('auth:web')
